...
- Added an optional repo map: with `repoMap.enabled`, the system prompt includes a ranked, budget-limited outline of the workspace's exported symbols (signatures) so the model reuses existing code instead of reinventing it (off by default; `repoMap.tokenBudget` sets the size).
