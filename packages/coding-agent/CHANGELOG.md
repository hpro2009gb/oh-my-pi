...
- Added weapon packs (`pi-minimal`, `opm-verify`, `pi-super`) selectable with `--preset`/`--weapon`, `/weapon`, and Settings → Tools → Weapons. Super Pi starts on `pi-super`; `omp` is unchanged.
