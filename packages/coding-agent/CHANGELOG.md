...
- Added `/walkthrough` to emit a markdown artifact with file-change and command-evidence citations from the current session (off by default via `walkthrough.enabled`)
