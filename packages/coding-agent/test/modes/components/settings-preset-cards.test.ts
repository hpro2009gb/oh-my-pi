...
describe("SettingsSelectorComponent preset cards", () => {
	it("renders each pack's learnedFrom string and maturity badge", () => {
		for (const card of listPresetCards()) {
			...
			expect(output).toContain("session next starts");
		}
	});

	it("applies the selected pack from the card without overriding configured keys", () => {
		...
		// Enter opens the apply submenu; Enter again confirms.
		comp.handleInput("\n");
		comp.handleInput("\n");
